...
export async function changeFunctionWithAttachment(account_id: string, privatekey: string, contract_id: any, method: string, args: any, attachedDeposit: BN, referrer: string, callbackUrl: string | undefined, gas = DEFAULT_FUNCTION_CALL_GAS) {
    try {
        const nearAccount = await getAccount(account_id, privatekey);
        ...
        const walletUrl = getSigningTransactionsWalletUrl([transaction], referrer, callbackUrl);
